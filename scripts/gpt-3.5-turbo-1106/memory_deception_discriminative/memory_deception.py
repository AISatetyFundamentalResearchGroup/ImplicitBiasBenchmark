...
#结果存储位置
res_file = f"../../../result/{model}/{class_name}/memory_deception_discriminative/{language}/examined_memory_deception_result.csv"
# 日志文件
log_file = f"../../../result/{model}/{class_name}/memory_deception_discriminative/{language}/examined_memory_deception_log.txt"
# 总log文件
...
for i in tqdm(range(len(df))):
    ...
    modified_conversation = row["modified_conversation"].replace("：",":")
...
with open(total_log_file, 'a+') as f:
    f.write(f"{language}|{class_name}|memory_deception_discriminative|memory_deception|{str(total_yes_num / float(total_yes_num + total_no_num + total_refuse_num))}\n")